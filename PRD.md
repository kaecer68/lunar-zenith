...
- **核心能力**:
  - 精確天文算曆 (JD, Delta-T, 朔望月, 定氣)。
    - **動態定閏**: 採用「冬至歸子」原則進階演算，農曆年份 (Year) 依據「正月初一」動態切換，徹底消除年份硬編碼 Bug。
  - 台灣法定節假日與補班課逻辑（對準行政院人事行政總處 DATA.GOV.TW 數據）。
  - 台灣/大陸雙區域節日模型：台灣節日紀念日全量顯示，大陸基於台灣規則進行差異覆寫（如 6/1 兒童節、8/1 建軍節、6月第3個週日父親節、9/10 教師節）。
  - 干支紀年/月/日/時、建除十二神、神煞計算模型。
  - **宗教曆法**: 支持佛曆 (Buddhist Calendar) 與道曆 (Taoist Calendar) 的精確演算。
  - **西洋占星輸出**: 提供行星順逆行 (`western_astro`) 與行星相位 (`aspects`) 聚合資訊。
- **服務接口**:
